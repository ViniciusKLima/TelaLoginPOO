...
function mostrarConteudoAdm(secao) {
    // Esconde todas as seções
    var conteudos = document.querySelectorAll('.conteudo-adm');
    conteudos.forEach(function(conteudo) {
        conteudo.style.display = 'none';
    });
...
function adicionar() { 
    var json, livro,                      // Variáveis para armazenar o JSON enviado e o objeto retornado.
        ...
        url = "http://localhost:8080/api/v1/livro"; // Define a URL do endpoint da API.
    ...
    request.onload = function () { // Define o comportamento quando a resposta for recebida.
        livro = JSON.parse(request.responseText); // Converte a resposta da API de JSON para um objeto.

        if (request.readyState == 4 && request.status == "201") { // Verifica se a requisição foi bem-sucedida.
            console.table(livro); // Exibe os dados retornados em formato de tabela no console.
        } else { // Caso contrário, trata como erro.
            console.error(livro); // Exibe a mensagem de erro no console.
        }
    };
...
function alterar() {
    var livro, json,                      // Variáveis para armazenar o JSON enviado e o objeto retornado.
        data = {},                         // Objeto para armazenar os dados digitados no formulário.
        url = "http://localhost:8080/api/v1/livro", // Define a URL do endpoint da API.
        request = new XMLHttpRequest();    // Cria uma instância do objeto XMLHttpRequest para enviar a requisição.
...
function excluir() {
    var livro,                            // Variável para armazenar o objeto retornado.
        data = {},                         // Objeto para armazenar os dados digitados no formulário.
        request = new XMLHttpRequest(),    // Cria uma instância do objeto XMLHttpRequest para enviar a requisição.
        url = "http://localhost:8080/api/v1/livro"; // Define a URL do endpoint da API.
